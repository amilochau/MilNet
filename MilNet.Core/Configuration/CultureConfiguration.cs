...
namespace MilNet.Core.Configuration
{
    ...
    public abstract class CultureConfiguration
    {
        ...
        private List<string> implementedCultures = new List<string>() { "en" };

        /// <summary>Constructor</summary>
        /// <param name="implementedCultures">Implemented cultures</param>
        /// <exception cref="ArgumentNullException"/>
        protected CultureConfiguration(params string[] implementedCultures)
        {
            if (implementedCultures == null)
                throw new ArgumentNullException(nameof(implementedCultures));

            foreach (string culture in implementedCultures)
                this.implementedCultures.Add(culture);
        }
        ...
        public string GetImplementedCulture(string name)
        {
            ...
            // Valid culture?
            if (validCultures.Where(c => c.Equals(name, StringComparison.OrdinalIgnoreCase)).Count() == 0)
                return DefaultCulture;

            // Implemented culture?
            if (implementedCultures.Where(c => c.Equals(name, StringComparison.OrdinalIgnoreCase)).Count() > 0)
                return name;

            // Nearest language
            string neutralCulture = GetNeutralCulture(name);
            foreach (string culture in implementedCultures)
                if (culture.StartsWith(neutralCulture, StringComparison.OrdinalIgnoreCase))
                    return culture;

            // If culture is not implemented: default culture
            return DefaultCulture;
        }

        /// <summary>Default culture ("en")</summary>
        public string DefaultCulture => implementedCultures[0];
        ...
        public static string CurrentNeutralCulture => GetNeutralCulture(Thread.CurrentThread.CurrentCulture.Name);

        /// <summary>Application neutral culture name</summary>
        public static string GetNeutralCulture(string name) => name != null && name.Contains("-") ? name.Split('-')[0] : name;
    }
}
